...
router.get('/edamam/:encodedUri', async (req, res) => {
    try {
        ...
        if (!EDAMAM_APP_ID || !EDAMAM_APP_KEY) {
            return res.status(500).json({ error: "Edamam API credentials are not configured correctly." });
        }
        const edamamApiUrl = `https://api.edamam.com/api/recipes/v2/by-uri?type=public&uri=${encodeURIComponent(decodedUri)}&app_id=${EDAMAM_APP_ID}&app_key=${EDAMAM_APP_KEY}`;
        ...
        console.log("Edamam API raw response data structure:", JSON.stringify(apiResponse.data, null, 2));

        let recipeData = null;


        if (apiResponse.data.hits && Array.isArray(apiResponse.data.hits) && apiResponse.data.hits.length > 0) {
        ...
        if (recipeData) {
            ...
            console.warn("Edamam API returned 200 OK, but no valid recipe data found in expected structure for URI:", decodedUri);
            res.status(404).json({ error: "Edamam recipe not found for the provided URI." });
        }
        ...
        if (e.response) {
            ...
            console.error('Edamam API Error Response Data:', JSON.stringify(e.response.data, null, 2));
        }
        res.status(e.response.status || 500).json({ error: e.message || "Failed to fetch Edamam recipe." });
    }
});
...
router.get('/user/:id', authenticate, async (req, res) => {
    try{
        ...
        res.status(500).json({ error: e.message || "Failed to fetch user recipe details." });
    }
})
